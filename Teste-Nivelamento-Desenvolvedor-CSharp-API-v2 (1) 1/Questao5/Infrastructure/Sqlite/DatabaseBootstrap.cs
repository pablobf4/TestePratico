...
namespace Questao5.Infrastructure.Sqlite
{
    public class DatabaseBootstrap : IDatabaseBootstrap
    {
        ...
        public void Setup()
        {
            ...
            var tables = connection.Query<string>("SELECT name FROM sqlite_master WHERE type='table' AND (name = 'contacorrente' or name = 'movimento' or name = 'idempotencia');").ToList();

            if (!tables.Contains("contacorrente"))
            {
            ...
            if (!tables.Contains("idempotencia"))
            {
                connection.Execute("CREATE TABLE idempotencia (" +
                         "chave_idempotencia INTEGER PRIMARY KEY AUTOINCREMENT," +
